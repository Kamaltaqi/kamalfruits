...
class MainApp(MDApp):
    ...
    def toggle_camera(self):
        try:
            if not self.is_camera_open:
                self.capture = Camera(play=True, resolution=(640, 480))
                self.root.ids.img.add_widget(self.capture)
    ...
    def capture_image(self):
        try:
            ...
            self.root.ids.img.source = image_path
            result = self.predict_fruit(image_path)
            self.root.ids.result_label.text = self.reshape_text(f"النتيجة: {result}")
            self.close_camera()
        except Exception as e:
    ...
    def close_camera(self):
        try:
            if self.capture:
                self.root.ids.img.remove_widget(self.capture)
                self.is_camera_open = False
